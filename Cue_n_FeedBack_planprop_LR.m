function [sys,x0,str,ts,simStateCompliance] = Cue_n_FeedBack_planprop_LR(t,x,u,flag,video3,Planning,hardwareConn,classLabels,cueAppear,initDelay,trialLength)
...
global ready prepLeft prepRight RightNow blank LeftNow  fdStart  tTrshld trlFlg imStatus scoreR scoreL arduino trlNum prevFrameR prevFrameL 
...
global ready prepLeft prepRight RightNow blank LeftNow fdStart tTrshld trlFlg trlStart trlInstant imStatus scoreR scoreL arduino trlNum prevFrameR prevFrameL
...
%                 
                ...
                elseif(trlInstant>=cueAppear && trlInstant<=cueAppear+1)
                    if(label==1)
                        if(imStatus(3)==0)
                            imshow(RightNow);imStatus=[0 0 1 0 0 0];scoreR=0;prevFrameR=0;
                        end
                    elseif(label==2)
                        if(imStatus(4)==0)
                            imshow(LeftNow);imStatus=[0 0 0 1 0 0];scoreL=0;prevFrameL=0;
                        end
                    end
                elseif(trlInstant>cueAppear+1 && trlInstant<trialLength)
                       if(label==1)
                            frameR=round(scoreR/150*100)  +1;
                                  %100 is the cropped frame length of the video out of 172 frames, 66 is the maximum score one can achive 0.30*66=2s(approx).
                            if(frameR>=1 && frameR<100)
                                if(frameR>prevFrameR+5)
                                    imshow(video3(:,:,:,frameR));
                                    prevFrameR=frameR;
                                end
                                rewardR=frameR*2;  %reward=(currentframe/totalframe)*100%
    %                             
                                if(hardwareConn==1) 
                                    fwrite(arduino,rewardR);
                                    if(rewardR>=100)
                                        scoreR=0;
                                        prevFrameR=0;
                                    end
                       ...
                       elseif(label==2)
                           frameL=round(scoreL/150*100)  +1;
                                  %100 is the cropped frame length of the video out of 172 frames, 66 is the maximum score one can achive 0.30*66=2s(approx).
                            if(frameL>=1 && frameL<100)
                                if(frameL>prevFrameL+5)
                                    imshow(video3(:,:,:,frameL));
                                    prevFrameL=frameL;
                                end
                                rewardL=frameL*2;  %reward=(currentframe/totalframe)*100%
    %                             
                                if(hardwareConn==1) 
%                                     fwrite(arduino,rewardL);
                                    if(rewardL>=100)
                                        scoreL=0;
                                        prevFrameL=0;
                                    end
